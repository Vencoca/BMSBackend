...
class TuyaCloudApiHandler {
    ...
    private getRequestOptions(timestamp: number, url: string, useAcessToken: boolean = false) {
        const sign = this.calcSign(this.clientId, timestamp, this.nonce, this.stringToSign(url), this.secret, useAcessToken ? this.access?.access_token : "");
    ...
    private async renewAccessToken(): Promise<void> {
        ...
        if (!this.access || now - this.lastTokenRefresh >= this.access!.expire_time + 120) {
    ...
    private calcSign(clientId: string, timestamp: number, nonce: string, signStr: string, secret: string, accessToken:string="") {
